...
def preflight(repo_root: Path, expected_sweep_root: Path) -> dict[str, object]:
    ...
    require('"VINE_OBSERVATION_MODE"', text["manifest_writer"], "manifest records mode environment", fixed=True)
    require("interventions = list(", text["manifest_writer"], "manifest has explicit intervention block", fixed=True)
    require("vine_observation_mode = vine_observation_mode", text["manifest_writer"], "manifest records normalized mode", fixed=True)
    require("no_vine_signal_mask = no_vine_signal_mask", text["manifest_writer"], "manifest records exact signal mask", fixed=True)
