...
/gabirel/miniforge3/bin/python3 \
  ...
  --output frozen_releases/post_holdout_secondary_plan_v13 \
  --bundle frozen_releases/post_holdout_secondary_plan_v13.tar.gz

(cd frozen_releases && \
  sha256sum -c post_holdout_secondary_plan_v13.tar.gz.sha256)
(cd frozen_releases/post_holdout_secondary_plan_v13 && \
...
least two, matching the frozen two-worker launcher. Set its exact path, then run:
...
## 6. Train the ten-seed policy-visible-vine-state negative control on two GPUs

The launcher gives each of two workers one A100 and 44 vine-simulation CPU
cores, using 88 of the 95 available cores and leaving seven for orchestration,
I/O, and the operating system. It refuses an existing output root and a silent
CPU fallback. The ten frozen seeds are divided deterministically five per
worker.

```bash
export RSCRIPT=/gabirel/miniforge3/bin/Rscript
export PYTHON=/gabirel/miniforge3/bin/python3
export NO_VINE_SWEEP_ROOT="$PWD/data/no_vine_rl_runs_secondary_v3"
export NO_VINE_WORKERS=2
export VINE_SIM_CORES_PER_WORKER=44
...
/gabirel/miniforge3/bin/python3 \
  ...
  --status data/no_vine_rl_runs_secondary_v3/seed_sweep_status.csv
```
...
/gabirel/miniforge3/bin/python3 \
  ...
  --rl-runs data/no_vine_rl_runs_secondary_v3 \
  --expected-seeds 10 \
...
/gabirel/miniforge3/bin/python3 \
  ...
  --secondary-plan-release frozen_releases/post_holdout_secondary_plan_v13 \
...
(cd secondary_evaluation/post_holdout_explanatory_ablation_v2_operational_retry && \
  sha256sum -c CONTENTS.sha256)

/gabirel/miniforge3/bin/python3 -m json.tool \
  secondary_evaluation/post_holdout_explanatory_ablation_v2_operational_retry/\
