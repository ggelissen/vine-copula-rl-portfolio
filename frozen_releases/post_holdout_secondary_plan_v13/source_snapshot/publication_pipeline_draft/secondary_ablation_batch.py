...
def validate_contract(contract: dict[str, Any]) -> dict[str, Any]:
    ...
    require_fields(
        matched,
        [
            "required_equal_code_paths", "allowed_config_difference_paths",
            "required_no_vine_source_markers", "require_equal_actor_parameter_count",
    ...
    allowed_config = matched["allowed_config_difference_paths"]
    if (
        not isinstance(allowed_config, list)
        or len(allowed_config) != len(set(allowed_config))
        or not all(isinstance(path, str) and path for path in allowed_config)
    ):
        raise SecondaryAblationError(
            "matched_design.allowed_config_difference_paths must be unique strings."
        )
    source_markers = matched["required_no_vine_source_markers"]
    if not isinstance(source_markers, dict) or not source_markers:
        raise SecondaryAblationError(
            "matched_design.required_no_vine_source_markers must be a non-empty object."
        )
    for path, markers in source_markers.items():
        safe_relative(str(path), "matched_design.required_no_vine_source_markers")
        if (
            ...
            raise SecondaryAblationError(
                f"No-vine source markers for {path} must be unique non-empty strings."
            )
    for field in [
...
def _read_seed_protocol_rows(
    ...
    seeds: list[int],
    expected_mode: str,
    allow_legacy_full_mode: bool = False,
    ...
    for seed in seeds:
        ...
        recorded_mode = report.get("vine_observation_mode")
        if recorded_mode == expected_mode:
            mode_evidence_status = "embedded_in_frozen_sanity_report"
        elif (
            expected_mode == "full"
            and allow_legacy_full_mode
            and recorded_mode in (None, "")
        ):
            # The original full-model release predates explicit ablation-mode
            # metadata.  Its frozen contract permits only an absent marker,
            # never a conflicting value.  Exact reproduction of the archived
            # v4 full-policy weights is enforced later in this same batch.
            mode_evidence_status = (
                "legacy_missing_allowed_by_frozen_contract_pending_exact_v4_replay"
            )
        else:
            raise SecondaryAblationError(
        ...
        rows.append(
            {
                ...
                "mode_evidence_status": mode_evidence_status,
...
def _frozen_source_text(release: Path, relative: str) -> str:
    normalized = safe_relative(relative, "frozen training source path")
    path = release / "source_snapshot" / Path(*normalized.parts)
    if not path.is_file():
        raise SecondaryAblationError(
            f"Frozen training release lacks source snapshot: {relative}"
        )
    return path.read_text(encoding="utf-8")


def _drop_dotted_path(value: dict[str, Any], dotted: str) -> None:
    parts = dotted.split(".")
    if not parts or any(not part for part in parts):
        raise SecondaryAblationError(f"Invalid allowed configuration path: {dotted!r}")
    current: Any = value
    for part in parts[:-1]:
...
def _normalized_frozen_config(
    release: Path, allowed_differences: list[str]
) -> dict[str, Any]:
    raw = yaml.safe_load(_frozen_source_text(release, "config/config.yaml"))
    if not isinstance(raw, dict):
        raise SecondaryAblationError("Frozen config/config.yaml must contain an object.")
    normalized = json.loads(json.dumps(raw))
    for dotted in allowed_differences:
        _drop_dotted_path(normalized, dotted)
    return normalized
...
def verify_matched_training_design(
    ...
    full, full_code = _read_seed_protocol_rows(
        full_release,
        full_seeds,
        "full",
        allow_legacy_full_mode=allow_legacy_full_mode,
    )
    ...
    rows.extend(
        [
            {
                "check": "protocol:full_vine_observation_mode_evidence",
                "full_value": json.dumps(
                    sorted(set(full["mode_evidence_status"].astype(str)))
                ),
                "ablation_value": "not_applicable",
                "status": "pass",
            },
            {
                "check": "protocol:no_vine_observation_mode_evidence",
                "full_value": "not_applicable",
                "ablation_value": json.dumps(
                    sorted(set(ablation["mode_evidence_status"].astype(str)))
                ),
                "status": "pass",
            },
        ]
    ...
    allowed_config = [
        str(path) for path in matched_contract["allowed_config_difference_paths"]
    ]
    full_config = _normalized_frozen_config(full_release, allowed_config)
    ablation_config = _normalized_frozen_config(no_vine_release, allowed_config)
    config_passed = full_config == ablation_config
    rows.append(
        {
            "check": "matched_design:config_equal_after_declared_operational_exclusions",
            "full_value": hashlib.sha256(
                json.dumps(full_config, sort_keys=True).encode("utf-8")
            ).hexdigest(),
            "ablation_value": hashlib.sha256(
                json.dumps(ablation_config, sort_keys=True).encode("utf-8")
            ).hexdigest(),
            "status": "pass" if config_passed else "fail",
        }
    )
    for path, markers in matched_contract["required_no_vine_source_markers"].items():
        full_hash = full_code.get(str(path))
        ablation_hash = ablation_code.get(str(path))
        ablation_source = _frozen_source_text(no_vine_release, str(path))
        missing_markers = [str(marker) for marker in markers if str(marker) not in ablation_source]
        passed = bool(full_hash and ablation_hash and not missing_markers)
        rows.append(
            {
                "check": f"controlled_ablation_source:{path}",
                "full_value": full_hash or "missing",
                "ablation_value": (
                    ablation_hash if not missing_markers
                    else "missing_markers:" + json.dumps(missing_markers)
                ),
                "status": "pass" if passed else "fail",
            }
        )
...
def execute_secondary_ablation(
    *,
    ...
    matched_design_report = verify_matched_training_design(
        ...
        allow_legacy_full_mode=bool(
            full_experiment.get("allow_legacy_missing_mode", False)
        ),
    )
