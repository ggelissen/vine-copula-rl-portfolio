...
def validate_panel(frame: pd.DataFrame, contract: dict[str, Any]) -> None:
    ...
    minimum_complete = int(contract["sample"]["minimum_complete_periods"])
    ...
    complete_counts = frame.groupby("strategy_id")["complete"].sum()
    require((complete_counts >= minimum_complete).all(),
            "At least one strategy has too few complete periods.")
    require(complete_counts.nunique() == 1,
            "Strategies do not share one common complete-period sample.")
    declared_complete = int(contract["sample"].get(
        "observed_complete_periods_before_scoring", complete_counts.iloc[0]))
    require(int(complete_counts.iloc[0]) == declared_complete,
            "Complete-period count differs from the disclosed analysis sample.")
    complete_calendar = frame[[
        "decision_date", "holding_end_date", "complete"]].drop_duplicates()
    require(len(complete_calendar) == expected_periods,
            "Complete-period flags are not common across strategies.")
...
def training_gate_diagnostics(frame: pd.DataFrame) -> pd.DataFrame:
    """Summarize intent-to-train gate evidence without selecting policies."""
    columns = [
        "experiment_id", "strategy_id", "seed", "protocol_eligibility_pass",
        "behavior_gate_pass", "behavior_gate_mode",
        "behavior_gate_failed_metrics", "operational_source",
    ]
    seed_rows = frame.loc[frame["strategy_level"] == "seed", columns].drop_duplicates()
    require(len(seed_rows) == 130 and seed_rows["strategy_id"].nunique() == 130,
            "Training-gate diagnostics require exactly 130 unique seed policies.")
    seed_rows = seed_rows.copy()
    seed_rows["protocol_eligibility_pass"] = parse_bool(
        seed_rows["protocol_eligibility_pass"], "protocol_eligibility_pass")
    seed_rows["behavior_gate_pass"] = parse_bool(
        seed_rows["behavior_gate_pass"], "behavior_gate_pass")
    require(seed_rows["protocol_eligibility_pass"].all(),
            "An evaluated causal policy failed protocol eligibility.")

    rows: list[dict[str, Any]] = []
    for experiment, group in seed_rows.groupby("experiment_id", sort=True):
        warning_metrics = sorted({
            metric
            for value in group.loc[
                ~group["behavior_gate_pass"], "behavior_gate_failed_metrics"]
            for metric in str(value).split(";") if metric
        })
        rows.append({
            "experiment_id": experiment,
            "seed_policy_count": len(group),
            "protocol_eligible_count": int(group["protocol_eligibility_pass"].sum()),
            "all_economic_diagnostics_pass_count": int(
                group["behavior_gate_pass"].sum()),
            "report_only_included_count": int(
                (group["behavior_gate_mode"] == "report_only").sum()),
            "economic_warning_count": int((~group["behavior_gate_pass"]).sum()),
            "economic_warning_fraction": float((~group["behavior_gate_pass"]).mean()),
            "economic_warning_metrics": ";".join(warning_metrics),
            "operational_sources": ";".join(sorted(
                set(group["operational_source"].astype(str)))),
            "selection_rule": "intent_to_train_no_economic_gate_selection",
        })
    result = pd.DataFrame(rows)
    require(len(result) == 13 and int(result["seed_policy_count"].sum()) == 130,
            "Training-gate diagnostic summary has the wrong cardinality.")
    return result
...
def write_plots(output: Path, frame: pd.DataFrame, metrics_frame: pd.DataFrame,
    ...
    # Matplotlib renamed ``labels`` to ``tick_labels`` and some releases no
    # longer accept the old keyword. Set ticks explicitly after constructing
    # the boxplot so this works across both API generations.
...
def analyze(contract_path: Path, period_panel: Path, output: Path) -> dict[str, Any]:
    ...
    primary_scope = validated.raw["sample"].get(
        "primary_sample_scope", "complete_periods")
    require(primary_scope == "complete_periods",
            "Causal accounting currently requires complete-period inference.")
    analysis_frame = frame[frame["complete"]].copy()
    require(not analysis_frame.empty,
            "The complete-period causal analysis sample is empty.")

    metric_rows: list[dict[str, Any]] = []
    group_columns = ["experiment_id", "strategy_id", "strategy_level"]
    for key, group in analysis_frame.groupby(
    ...
    primary = contrast_rows(analysis_frame, validated.raw,
                            validated.raw["primary_component_contrasts"],
                            "primary_component", two_sided=False)
    algorithms = contrast_rows(analysis_frame, validated.raw,
                               validated.raw["algorithm_robustness_contrasts"],
                               "algorithm_robustness", two_sided=True)
    seed_pairs, seed_summary = seed_effects(analysis_frame, validated.raw)
    gate_diagnostics = training_gate_diagnostics(analysis_frame)
    ...
    try:
        tables = {
            ...
            "causal_training_gate_diagnostics.csv": gate_diagnostics,
        }
        for name, table in tables.items():
            table.to_csv(temporary / name, index=False)
        write_plots(temporary, analysis_frame, metric_frame, primary, seed_pairs,
                    validated.raw)
        manifest = {
            ...
            "analysis_period_rows": len(analysis_frame),
            "locked_periods_per_strategy": int(validated.raw["sample"][
                "expected_periods"]),
            "complete_periods_per_strategy": int(
                analysis_frame.groupby("strategy_id").size().iloc[0]),
            "primary_sample_scope": primary_scope,
            "strategy_count": frame["strategy_id"].nunique(),
            ...
            "protocol_eligible_policy_count": int(
                gate_diagnostics["protocol_eligible_count"].sum()),
            "all_economic_diagnostics_pass_count": int(
                gate_diagnostics["all_economic_diagnostics_pass_count"].sum()),
            "report_only_included_count": int(
                gate_diagnostics["report_only_included_count"].sum()),
        ...
        shutil.copy2(contract_path, temporary / "causal_analysis_contract.json")
