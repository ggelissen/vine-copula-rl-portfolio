...
def verify_causal_analysis_release(release: Path, repo_root: Path) -> dict[str, Any]:
    ...
    if manifest.get("release_status") != "frozen_pre_causal_evaluation_analysis_plan":
...
def freeze(repo_root: Path, extension_release: Path, contract_path: Path,
    ...
    try:
        ...
        manifest = {
            "schema_version": 1,
            "release_status": "frozen_pre_causal_evaluation_analysis_plan",
            ...
            "operational_revision_reason": (
                "The first replay attempt exported zero policies because the "
                "general evaluator required per-run sanity reports that were not "
                "part of the frozen causal training protocol. Replay v2 accepts "
                "only exact checkpoint hashes authorized by the passed centralized "
                "130-checkpoint audit."),
            ...
            "next_action": "audit training, generate weights, construct ensembles, and score once",
        }
        (temporary / "causal_analysis_release_manifest.json").write_text(
            json.dumps(manifest, indent=2, sort_keys=True) + "\n", encoding="utf-8")
        (temporary / "READ_ONLY_RELEASE.txt").write_text(
            "Do not edit. This freezes post-holdout explanatory analysis before causal evaluation.\n",
            encoding="utf-8")
        lines = []
