#!/usr/bin/env bash
# Run the frozen ten-seed no-vine experiment on independent GPU workers.
set -euo pipefail
...
CORES_PER_WORKER="${VINE_SIM_CORES_PER_WORKER:-44}"
...
if torch.cuda.device_count() < required_gpus:
    raise SystemExit(
        f"{required_gpus} GPUs are required by this launcher; "
...
"$RSCRIPT" --vanilla -e \
  "parse(file='rl/train_rl.r'); parse(file='rl/rl_environment.r')" \
  >/dev/null

preflight_json="$(mktemp /tmp/no-vine-preflight.XXXXXX.json)"
trap 'rm -f "$preflight_json"' EXIT
"$PYTHON" publication_pipeline_draft/preflight_no_vine_training_contract.py \
  --repo-root "$REPO_ROOT" \
  --sweep-root "$SWEEP_ROOT" >"$preflight_json"
cat "$preflight_json"

mkdir -p "$SWEEP_ROOT/worker_logs"
cp "$preflight_json" "$SWEEP_ROOT/training_contract_preflight.json"
declare -a pids=()
...
"$TRAIN_PYTHON" publication_pipeline_draft/verify_no_vine_training_evidence.py \
  ...
  --require-embedded >"$SWEEP_ROOT/checkpoint_evidence.json"

status_inputs=()
for worker in $(seq 1 "$WORKERS"); do
  status_inputs+=("$SWEEP_ROOT/worker_status_${worker}.csv")
done
