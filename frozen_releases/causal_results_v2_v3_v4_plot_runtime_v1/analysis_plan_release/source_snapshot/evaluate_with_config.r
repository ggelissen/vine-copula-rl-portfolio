...
# its local no-holdout sanity report.  The later causal study uses a different,
# frozen gate: one centralized audit binds every permitted checkpoint hash to
# its preregistered architecture and gate policy.  That audited path is enabled
# only by generate_causal_policy_weights.py and remains weights-only.
...
causal_gate_authorization <- Sys.getenv("EVAL_GATE_AUTHORIZATION", unset = "")
audited_causal_replay <- identical(
  causal_gate_authorization, "causal_checkpoint_audit_v1")
