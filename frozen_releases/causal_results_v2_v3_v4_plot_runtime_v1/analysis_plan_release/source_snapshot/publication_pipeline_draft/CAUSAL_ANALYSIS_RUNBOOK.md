...
## Operational replay v2 after the zero-output preflight failure

The first causal replay attempt stopped before policy inference because the
general evaluation launcher expected a per-run `sanity_no_holdout` report.
Those reports were not outputs of the frozen 130-job causal training protocol.
The attempt exported zero weight files and therefore produced no causal return
outcomes.  Do not delete or overwrite that failed directory.

Replay v2 does not waive training validation.  It replaces that inapplicable
local-artifact check with the already passed centralized checkpoint audit.  The
launcher permits this path only for weights-only replay of a full checkpoint,
and the checkpoint must match exactly one audited path, SHA-256 digest, finite
tensor result, behavior-gate mode, and the disclosed 70-v2/31-v3/29-v4 audit.
Ordinary evaluation continues to require its local no-holdout sanity report.

The revised immutable paths are:

```text
frozen_releases/causal_analysis_v2_v3_v4_replay_v2
...
analysis_outputs/causal_common_accounting_v2_v3_v4_replay_v2
analysis_outputs/causal_analysis_results_v2_v3_v4_replay_v2
frozen_releases/causal_results_v2_v3_v4_replay_v2
```

The existing `analysis_outputs/causal_sweep_audit_v2_v3_v4` is reused without
modification.  Run the staged driver from `freeze-plan` onward; do not rerun
training, merging, or checkpoint auditing.

## Accounting v2 calendar-completeness correction

The replay-v2 accounting attempt stopped before scoring because its adapter
required all 24 locked periods to have the conservative complete-month flag.
The immutable realized panel contains 24 periods and 22 complete periods: the
February 2026 interval has 14 trading days and the terminal July 2026 interval
is intentionally shortened.  The already frozen main-evaluation contract
requires 24 locked periods and at least 20 complete periods, with inference
restricted to complete periods.

The corrected causal contract inherits that exact rule.  It does not relabel
the two shortened periods, change any date or return, or change hypotheses,
weights, costs, estimands, bootstrap settings, or decision rules.  Because the
first replay completed before this issue was found, accounting v2 explicitly
records that evaluation returns were accessed during replay but that no causal
strategy return, metric, contrast, table, or figure had been scored.  For a
clean hash chain, accounting v2 replays the audited checkpoints and rebuilds
the ensembles under the corrected contract; training and checkpoint auditing
are not repeated.

The accounting-v2 immutable paths end in `_accounting_v2`, and its causal
contract is `publication_pipeline_draft/config/causal_analysis_contract_v2.json`.

### Matplotlib plotting-runtime revision

Accounting v2 subsequently completed common scoring and standardized-panel
export, but plot construction failed because the installed Matplotlib rejected
the former `Axes.boxplot(labels=...)` API.  The runtime-only revision constructs
the same boxplot without a labels keyword and assigns the identical tick labels
afterwards.  No data, statistic, contrast, multiplicity adjustment, decision
rule, or visual encoding changes.  The revision is frozen after scoring and is
therefore explicitly marked as such.  The staged driver uses `_plot_runtime_v1`
for the new analysis release, interface, accounting output, panel, results, and
final archive while reusing the already audited accounting-v2 weights and
ensembles.
