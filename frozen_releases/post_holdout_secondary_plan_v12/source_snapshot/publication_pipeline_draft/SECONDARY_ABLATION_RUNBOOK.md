...
The legacy v2 sweep is invalidated: its hash-matched trainer and environment
sources did not implement the zero-mode propagation and masking path. It is
failure provenance only and must never be evaluated as a no-vine model. The
clean v3 sweep runs a source/config preflight before creating its output root,
and it requires embedded mode/mask metadata in all checkpoints before merging
statuses.
