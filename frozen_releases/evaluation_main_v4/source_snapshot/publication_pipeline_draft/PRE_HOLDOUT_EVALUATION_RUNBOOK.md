...
Evaluation release v4 is an operational revision after four archived attempts
failed before producing policy weights: R Lantern and Python PyTorch exported
incompatible libtorch symbols in one process. The v4 evaluator runs the
...
POLICY_PYTHON=/gabirel/venvs/copula-eval-torch271-cpu/bin/python \
EVAL_MODEL_DIR=frozen_releases/training_schema5_v1/seeds/seed_20260742 \
EVAL_OUTPUT_DIR="$DRY_RUN_DIR" \
EVAL_WEIGHTS_ONLY=true \
EVAL_DEVELOPMENT_DRY_RUN=true \
EVAL_WINDOW_ID=development_preflight_v4 \
/gabirel/miniforge3/bin/Rscript --vanilla \
  evaluate_with_config.r config/config.yaml \
  frozen_releases/training_schema5_v1/seeds/seed_20260742
...
matches = sorted(Path(os.environ["DRY_RUN_DIR"]).glob("weights_rl_full_*.csv"))
assert len(matches) == 1, f"Expected one full-policy weight file; found {matches}"
path = matches[0]
...
python3 publication_pipeline_draft/freeze_evaluation_release.py \
  ...
  --output frozen_releases/evaluation_main_v4 \
  --bundle frozen_releases/evaluation_main_v4.tar.gz

(cd frozen_releases && sha256sum -c evaluation_main_v4.tar.gz.sha256)
(cd frozen_releases/evaluation_main_v4 && sha256sum -c CONTENTS.sha256)
python3 -m json.tool frozen_releases/evaluation_main_v4/evaluation_release_manifest.json
```
...
/gabirel/miniforge3/bin/python3 \
  ...
  --output locked_evaluation/main_oos_v4_operational_retry \
  --bundle locked_evaluation/main_oos_v4_operational_retry.tar.gz \
  --rscript /gabirel/miniforge3/bin/Rscript

(cd locked_evaluation && sha256sum -c main_oos_v4_operational_retry.tar.gz.sha256)
python3 -m json.tool locked_evaluation/main_oos_v4_operational_retry/locked_batch_manifest.json
```

Only after the bundle and sidecar hash exist should tables and plots under
`locked_evaluation/main_oos_v4_operational_retry/publication_results` be inspected.
