...
def runtime_contract(obs_dim: int, action_dim: int) -> tuple[dict[str, object], dict[str, float | int]]:
...
def load_actor(checkpoint_path: Path, obs_dim: int, action_dim: int) -> tuple[LSTMActor, dict[str, float | int]]:
    ...
    actual.setdefault("vine_observation_mode", "full")
    mismatches = {
...
def run_server(args: argparse.Namespace) -> None:
    ...
    actor, projection = load_actor(args.checkpoint.resolve(), args.obs_dim, args.action_dim)
    ready = {
