...
NO_VINE_SWEEP_ROOT='data/no_vine_rl_runs_secondary_v1' RSCRIPT="$RSCRIPT" PYTHON="$PYTHON" \
...
"$PYTHON" publication_pipeline_draft/secondary_experiment_protocol.py \
  ...
  --status 'data/no_vine_rl_runs_secondary_v1/seed_sweep_status.csv'

# 3. Aggregate/freeze only after all expected gates pass.
"$PYTHON" publication_pipeline_draft/diagnostic_artifacts.py \
  --rl-runs 'data/no_vine_rl_runs_secondary_v1' --expected-seeds 10 \
...
"$PYTHON" publication_pipeline_draft/freeze_training_release.py \
  --repo-root . --rl-runs 'data/no_vine_rl_runs_secondary_v1' \
