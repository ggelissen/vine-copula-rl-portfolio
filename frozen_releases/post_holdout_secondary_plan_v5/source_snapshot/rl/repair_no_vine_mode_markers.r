...
for (index in seq_along(seeds)) {
  ...
  required_launcher_tokens <- c(
    ...
    "set_default_env(\"VINE_OBSERVATION_MODE\"")
  required_training_tokens <- c(
    "Sys.getenv(\"VINE_OBSERVATION_MODE\", \"full\")",
  ...
  if (any(!vapply(required_runner_tokens, grepl, logical(1),
                  x = snapshot_text[["rl/run_seed_sweep.r"]], fixed = TRUE)) ||
      any(!vapply(required_launcher_tokens, grepl, logical(1),
                  x = snapshot_text[["run_with_config.r"]], fixed = TRUE)) ||
      any(!vapply(required_training_tokens, grepl, logical(1),
