...
python3 publication_pipeline_draft/freeze_evaluation_release.py \
  ...
  --bundle frozen_releases/evaluation_main_v1.tar.gz

sha256sum -c frozen_releases/evaluation_main_v1.tar.gz.sha256
(cd frozen_releases/evaluation_main_v1 && sha256sum -c CONTENTS.sha256)
python3 -m json.tool frozen_releases/evaluation_main_v1/evaluation_release_manifest.json
```
...
```bash
python3 publication_pipeline_draft/locked_evaluation_batch.py \
  --repo-root . \
  --evaluation-release frozen_releases/evaluation_main_v1 \
  --output locked_evaluation/main_oos_v1 \
  --bundle locked_evaluation/main_oos_v1.tar.gz

sha256sum -c locked_evaluation/main_oos_v1.tar.gz.sha256
python3 -m json.tool locked_evaluation/main_oos_v1/locked_batch_manifest.json
```

Only after the bundle and sidecar hash exist should tables and plots under
`locked_evaluation/main_oos_v1/publication_results` be inspected.
