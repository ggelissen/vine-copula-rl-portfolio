...
def verify_frozen_extension_integrity(release: Path) -> dict[str, Any]:
    """Verify an immutable historical release without requiring a live mirror."""
...
def verify_extension_release(release: Path, repo_root: Path) -> dict[str, Any]:
    ...
    result = verify_frozen_extension_integrity(release)
    inventory_path = release / "source_inventory.csv"
    with inventory_path.open(newline="", encoding="utf-8") as stream:
        inventory = list(csv.DictReader(stream))
    for row in inventory:
