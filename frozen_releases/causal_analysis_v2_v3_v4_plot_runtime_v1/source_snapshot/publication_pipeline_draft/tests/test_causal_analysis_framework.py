...
def test_accounting_v2_contract_discloses_pre_scoring_calendar_correction() -> None:
    validated = load_contract(CORRECTED_CONTRACT)
    raw = validated.raw
    assert raw["analysis_status"] == \
        "operational_correction_before_causal_outcome_scoring"
    assert raw["causal_policy_replay_completed_before_revision"] is True
    assert raw["causal_performance_outcomes_scored_before_revision"] is False
    assert raw["sample"]["expected_periods"] == 24
    assert raw["sample"]["minimum_complete_periods"] == 20
    assert raw["sample"]["observed_complete_periods_before_scoring"] == 22
    assert raw["sample"]["primary_sample_scope"] == "complete_periods"


def test_causal_materializer_inherits_frozen_sample_completeness_rule() -> None:
    source = (ROOT /
        "publication_pipeline_draft/materialize_causal_evaluation.py").read_text(
            encoding="utf-8")
    assert 'contract.raw["sample"][\n            "minimum_complete_periods"]' in source
    assert '"minimum_complete_periods_per_window": 24' not in source
    analyzer = (ROOT /
        "publication_pipeline_draft/analyze_causal_results.py").read_text(
            encoding="utf-8")
    assert 'analysis_frame = frame[frame["complete"]].copy()' in analyzer
    assert "Incomplete periods are forbidden by this contract" not in analyzer
...
def test_analysis_freeze_source_closure_exists() -> None:
    ...
    for runtime in (
        "evaluate_with_config.r", "config/config.yaml", "rl/evaluate_rl.r",
        "rl/rl_environment.r", "rl/policy_inference_server_v2.py",
        "rl/recurrent_baselines.py", "rl/action_projection.py",
    ):
        assert runtime in SOURCES


def test_causal_replay_uses_exact_centralized_audit_authorization() -> None:
    launcher = (ROOT / "evaluate_with_config.r").read_text(encoding="utf-8")
    generator = (ROOT /
        "publication_pipeline_draft/generate_causal_policy_weights.py").read_text(
            encoding="utf-8")
    token = "causal_checkpoint_audit_v1"
    assert token in launcher and token in generator
    assert "EVAL_CAUSAL_CHECKPOINT_SHA256" in launcher
    assert "EVAL_CAUSAL_CHECKPOINT_SHA256" in generator
    assert "Audited causal replay is restricted to weights-only full checkpoints" in launcher
    assert "sanity_no_holdout" in launcher  # ordinary evaluation remains gated
...
def test_boxplot_labels_are_matplotlib_version_independent() -> None:
    source = (ROOT /
        "publication_pipeline_draft/analyze_causal_results.py").read_text(
            encoding="utf-8")
    assert "ax.boxplot(values, showmeans=True)" in source
    assert "ax.set_xticklabels(labels)" in source
    assert "labels=labels" not in source
    assert "tick_labels=labels" not in source
