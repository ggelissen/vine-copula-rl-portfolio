...
try:
    from .behavior_gate_protocol import validate_report_only_trainer
...
SOURCES = (
    ...
    "publication_pipeline_draft/behavior_gate_protocol.py",
    ...
    "publication_pipeline_draft/PUBLICATION_EXTENSION_V3_RECOVERY.md",
    ...
    "publication_pipeline_draft/V3_RETRY60_DIAGNOSIS.md",
    ...
    "publication_pipeline_draft/tests/test_behavior_gate_protocol.py",
...
def freeze(repo_root: Path, jobs: Path, runtime: Path, bundle_manifest: Path,
           ...
           training_output_root: Path = Path("data/publication_extension_runs_v2")) -> dict[str, object]:
    ...
    expected, contract_digest = validated_rows(
        contract_path, training_output_root)
    if any(row.get("PRETRAIN_BEHAVIOR_GATE_MODE") == "report_only"
           for row in expected):
        validate_report_only_trainer(repo_root / "rl/train_rl.r")
    ...
    for row in bundle_rows:
        ...
        if not bundle_path.is_file():
            raise FreezeError(f"Ablation bundle was not found: {bundle_path}")
        actual_bundle_sha256 = sha256(bundle_path)
        expected_bundle_sha256 = row["sha256"].lower()
        if actual_bundle_sha256 != expected_bundle_sha256:
            raise FreezeError(
                "Ablation bundle hash mismatch: "
                f"{bundle_path}; expected={expected_bundle_sha256}; "
                f"actual={actual_bundle_sha256}"
            )
...
def main() -> int:
    ...
    parser.add_argument("--training-output-root", type=Path, default=Path(
