...
read_benchmark_contract <- function(path) {
  ...
  contract$optimizer_ftol_rel <- as.numeric(
    contract$optimizer_ftol_rel %||% 0)
  if (length(contract$optimizer_ftol_rel) != 1L ||
      !is.finite(contract$optimizer_ftol_rel) ||
      contract$optimizer_ftol_rel < 0) {
    benchmark_protocol_error(
      "optimizer_ftol_rel must be a finite non-negative scalar.")
  }
  contract$optimizer_ftol_abs <- as.numeric(
    contract$optimizer_ftol_abs %||% 0)
  if (length(contract$optimizer_ftol_abs) != 1L ||
      !is.finite(contract$optimizer_ftol_abs) ||
      contract$optimizer_ftol_abs < 0) {
    benchmark_protocol_error(
      "optimizer_ftol_abs must be a finite non-negative scalar.")
  }
    ...
    result <- tryCatch(
      solver(
        ...
        control = list(
          ...
          ftol_rel = as.numeric(contract$optimizer_ftol_rel %||% 0),
          ftol_abs = as.numeric(contract$optimizer_ftol_abs %||% 0),
...
generate_vine_optimizers <- function(daily_log_returns, periods, contract,
                                     ...
                                     methods = c("static_vine", "rolling_vine",
                                                 "dynamic_nn_vine")) {
  ...
  if (!length(methods) || any(!methods %in% names(kinds)) ||
      anyDuplicated(methods)) {
    benchmark_protocol_error("Requested vine benchmark subset is invalid.")
  }
  kinds <- kinds[methods]
