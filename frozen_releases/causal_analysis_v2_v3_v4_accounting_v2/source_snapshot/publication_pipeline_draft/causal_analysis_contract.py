...
def load_contract(path: Path) -> ValidatedContract:
    ...
    require(analysis_status in {
        "prospective_before_causal_evaluation",
        "operational_correction_before_causal_outcome_scoring",
    }, "Analysis plan has an unsupported timing status.")
    ...
    if analysis_status == "prospective_before_causal_evaluation":
        require(raw.get("training_diagnostics_accessed_before_revision") is True and
                ...
                "The training-diagnostic revision disclosure is incomplete.")
    else:
        require(raw.get("training_diagnostics_accessed_before_revision") is True and
                raw.get("causal_evaluation_returns_accessed_before_revision") is True and
                raw.get("causal_policy_replay_completed_before_revision") is True and
                raw.get("causal_performance_outcomes_scored_before_revision") is False and
                raw.get("operational_revision") ==
                "accounting_v2_locked_calendar_completeness_alignment",
                "The pre-scoring accounting correction disclosure is incomplete.")
    require(raw.get("mixed_revision_carry_forward_permitted") is True,
            "The disclosed mixed-revision carry-forward rule is missing.")
    ...
    require(int(sample.get("expected_periods", 0)) == 24,
            "The locked causal window must retain exactly 24 periods.")
    require(1 <= int(sample.get("minimum_complete_periods", 0)) <= 24,
            "The minimum complete-period requirement is invalid.")
    if analysis_status == "operational_correction_before_causal_outcome_scoring":
        evaluation_contract_path = root / str(raw.get(
            "locked_evaluation_contract", ""))
        require(evaluation_contract_path.is_file(),
                "Referenced locked-evaluation contract is missing.")
        require(sha256(evaluation_contract_path) == str(raw.get(
            "expected_locked_evaluation_contract_sha256", "")),
            "Locked-evaluation contract hash differs from the correction plan.")
        evaluation_contract = json.loads(evaluation_contract_path.read_text(
            encoding="utf-8"))
        require(int(sample["expected_periods"]) == int(
                    evaluation_contract["expected_locked_periods_per_window"]) and
                int(sample["minimum_complete_periods"]) == int(
                    evaluation_contract["minimum_complete_periods_per_window"]) and
                sample.get("primary_sample_scope") ==
                    evaluation_contract["primary_sample_scope"],
                "Causal sample correction does not match the locked main evaluation.")
    ...
    try:
        if args.command == "validate":
            ...
            result = {"analysis_id": contract.raw["analysis_id"],
                      ...
                      "status": (
                          "valid_pre_scoring_operational_correction"
                          if contract.raw["analysis_status"] ==
                          "operational_correction_before_causal_outcome_scoring"
                          else "valid_prospective_analysis_plan")}
        else:
