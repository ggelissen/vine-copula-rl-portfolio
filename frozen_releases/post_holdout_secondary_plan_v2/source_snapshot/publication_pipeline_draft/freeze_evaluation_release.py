...
EVALUATION_CONFIG_SOURCES = frozenset(
    {
        "publication_pipeline_draft/config/evaluation_contract.json",
        "publication_pipeline_draft/config/benchmark_contract.json",
        "config/config.yaml",
...
def evaluation_source_role(relative: str) -> str:
    normalized = Path(relative).as_posix()
    if normalized in EVALUATION_CONFIG_SOURCES:
        return "config"
    if Path(normalized).suffix.lower() in {".py", ".r"}:
        return "code"
    return "supporting_source"


def hash_inventory_rows(rows: list[dict[str, Any]]) -> str:
    payload = "\n".join(
...
def evaluation_hash_aggregates(rows: list[dict[str, Any]]) -> dict[str, str]:
    normalized = [
        {**row, "role": row.get("role") or evaluation_source_role(str(row["path"]))}
        for row in rows
    ]
    return {
        "evaluation_code_sha256": hash_inventory_rows(
            [row for row in normalized if row["role"] == "code"]
        ),
        "evaluation_config_sha256": hash_inventory_rows(
            [row for row in normalized if row["role"] == "config"]
        ),
        "evaluation_source_aggregate_sha256": hash_inventory_rows(normalized),
    }
    ...
    try:
        ...
        for relative in EVALUATION_SOURCES:
            ...
            inventory.append(
                {
                    "path": relative,
                    "role": evaluation_source_role(relative),
        ...
        aggregate_hashes = evaluation_hash_aggregates(inventory)
        manifest = {
            "schema_version": 1,
            ...
            **aggregate_hashes,
            # Compatibility alias for already-frozen v4 consumers. New code
            # must use the role-specific hashes above.
            "evaluation_code_contract_sha256": aggregate_hashes[
            ...
            "operational_revision_reason": (
                "R Lantern and Python PyTorch are isolated in separate processes "
                "to prevent shared-library symbol collisions; full-mode evaluation "
                "supports the pre-ablation RLEnvironment constructor signature; "
                "each frozen seed directory is passed explicitly; heterogeneous "
                "benchmark audit columns are combined by name."
            ),
        ...
        contents_sha256 = sha256_file(temporary / "CONTENTS.sha256")
        (temporary / "CONTENTS.sha256.sha256").write_text(
            f"{contents_sha256}  CONTENTS.sha256\n", encoding="utf-8"
        )
