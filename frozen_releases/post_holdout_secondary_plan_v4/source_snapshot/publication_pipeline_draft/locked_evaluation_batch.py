...
try:  # package import in tests; script import in production CLI
    from .freeze_evaluation_release import evaluation_hash_aggregates
...
def verify_frozen_sources(repo_root: Path, release: Path) -> dict[str, Any]:
    ...
    hash_schema = int(manifest.get("evaluation_hash_schema_version", 1))
    if hash_schema >= 2:
        for field, actual in aggregates.items():
            if manifest.get(field) != actual:
                raise ProtocolError(
                    f"Frozen evaluation {field} mismatch: {manifest.get(field)} != {actual}"
                )
        if manifest.get("evaluation_code_contract_sha256") != aggregates[
            "evaluation_source_aggregate_sha256"
        ]:
            raise ProtocolError("Legacy evaluation aggregate alias is inconsistent.")
        sidecar_name = manifest.get("evaluation_contents_sha256_sidecar")
        sidecar = release / str(sidecar_name)
        if not sidecar.is_file():
            raise ProtocolError("Frozen evaluation CONTENTS hash sidecar is missing.")
        fields = sidecar.read_text(encoding="utf-8").strip().split()
        if not fields or fields[0] != contents_sha256:
            raise ProtocolError("Frozen evaluation CONTENTS hash role mismatch.")
    else:
        legacy = manifest.get("evaluation_code_contract_sha256")
        if legacy != aggregates["evaluation_source_aggregate_sha256"]:
            raise ProtocolError("Legacy frozen evaluation aggregate mismatch.")
    manifest = dict(manifest)
    manifest.update(
        {
            "evaluation_hash_schema_version": hash_schema,
            "evaluation_contents_sha256": contents_sha256,
            "evaluation_source_aggregate_sha256": manifest.get(
                "evaluation_source_aggregate_sha256",
                manifest["evaluation_code_contract_sha256"],
            ),
            "evaluation_code_sha256": manifest.get(
                "evaluation_code_sha256", manifest["evaluation_code_contract_sha256"]
            ),
            "evaluation_config_sha256": manifest.get(
                "evaluation_config_sha256", manifest["evaluation_code_contract_sha256"]
            ),
        }
    )
    return manifest
    ...
    for item in full_rows:
        ...
        rows.append(
            manifest_row(
                ...
                config_sha256=frozen_config_hash, code_sha256=frozen_code_hash,
...
def _publish_tree(temporary: Path, output: Path, bundle: Path) -> None:
    """Stage the complete archive before committing the immutable directory."""
    deterministic_tar(temporary, bundle, root_name=output.name)
    try:
        os.replace(temporary, output)
    except Exception:
        # These paths were proven absent before execution, so both files belong
        # to this failed publication attempt and are safe to roll back.
        _remove_bundle_pair(bundle)
        raise


def _preserve_failed_tree(temporary: Path, output: Path, bundle: Path) -> Exception | None:
    """Commit failure evidence even when compression itself is unavailable."""
    archive_error: Exception | None = None
    try:
        deterministic_tar(temporary, bundle, root_name=output.name)
    except Exception as error:  # directory evidence is the irreducible record
        archive_error = error
        _remove_bundle_pair(bundle)
    os.replace(temporary, output)
...
def execute_batch(args: argparse.Namespace) -> dict[str, Any]:
    ...
    try:
        ...
        create_strategy_manifest(
            ...
            timings,
            str(frozen["evaluation_code_sha256"]),
            str(frozen["evaluation_config_sha256"]),
        )
        ...
        status = {
            ...
            "evaluation_hash_schema_version": frozen["evaluation_hash_schema_version"],
            "evaluation_source_aggregate_sha256": frozen[
                "evaluation_source_aggregate_sha256"
            ],
            "evaluation_code_sha256": frozen["evaluation_code_sha256"],
            "evaluation_config_sha256": frozen["evaluation_config_sha256"],
            "evaluation_contents_sha256": frozen["evaluation_contents_sha256"],
