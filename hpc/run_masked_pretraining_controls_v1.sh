...
case "${1:-}" in
  ...
  checkpoint-archive)
    ...
    ;;
  finalize)
    if ! test -f "$RESULTS/masked_pretraining_analysis_manifest.json"; then
      echo "FINALIZE FAILURE: analysis manifest is missing; run a successful analyze stage first." >&2
      exit 1
    fi
    test ! -e "$ARCHIVE"
    tar -czf "$ARCHIVE" \
