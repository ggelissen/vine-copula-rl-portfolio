...
CAUSAL_CONTRACT=publication_pipeline_draft/config/causal_analysis_contract_v2.json
...
# Accounting-v2 paths preserve both earlier operational attempts.  The
# checkpoint audit is unchanged and remains the authoritative 130-checkpoint
# gate; policy weights and ensembles are replayed only to create a clean hash
# chain under the corrected pre-scoring calendar contract.
PLAN_RELEASE=frozen_releases/causal_analysis_v2_v3_v4_plot_runtime_v1
AUDIT=analysis_outputs/causal_sweep_audit_v2_v3_v4
WEIGHTS=analysis_outputs/causal_policy_weights_v2_v3_v4_accounting_v2
ENSEMBLES=analysis_outputs/causal_policy_ensembles_v2_v3_v4_accounting_v2
INTERFACE=analysis_outputs/causal_evaluation_interface_v2_v3_v4_plot_runtime_v1
ACCOUNTING=analysis_outputs/causal_common_accounting_v2_v3_v4_plot_runtime_v1
PANEL=analysis_outputs/causal_strategy_periods_v2_v3_v4_plot_runtime_v1.csv
RESULTS=analysis_outputs/causal_analysis_results_v2_v3_v4_plot_runtime_v1
RESULT_RELEASE=frozen_releases/causal_results_v2_v3_v4_plot_runtime_v1
...
freeze_plan() {
  ...
  local contrast_plan=protocol_manifests/causal_contrast_plan_v2_v3_v4_accounting_v2.csv
  ...
  (cd frozen_releases && sha256sum -c causal_analysis_v2_v3_v4_plot_runtime_v1.tar.gz.sha256)
}
...
common_accounting() {
  ...
  if test -e "$INTERFACE"; then
    require_file "$INTERFACE/CONTENTS.sha256"
...
analyze_results() {
  ...
  if test -f "$PANEL" && test -f "${PANEL}.manifest.json"; then
    "$PYTHON" -c 'import hashlib,json,sys; from pathlib import Path; panel=Path(sys.argv[1]); manifest=Path(str(panel)+".manifest.json"); evidence=json.loads(manifest.read_text(encoding="utf-8")); digest=hashlib.sha256(panel.read_bytes()).hexdigest(); assert evidence.get("status")=="causal_period_panel_complete"; assert evidence.get("causal_period_panel_sha256")==digest; assert int(evidence.get("rows",-1))==3432; assert int(evidence.get("strategies",-1))==143; print(f"Validated resumable causal period panel: {panel} ({digest})")' \
      "$PANEL"
  else
    require_absent "$PANEL"
    require_absent "${PANEL}.manifest.json"
...
freeze_results() {
  ...
  (cd frozen_releases && sha256sum -c causal_results_v2_v3_v4_plot_runtime_v1.tar.gz.sha256)
}
