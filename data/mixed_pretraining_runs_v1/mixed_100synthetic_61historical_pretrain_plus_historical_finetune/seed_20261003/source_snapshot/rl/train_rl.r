...
if (!is.null(metadata)) {
  expected_source <- switch(
    ...
    mixed_historical_synthetic = "historical_synthetic_mixture")
  if (identical(pretrain_data_mode, "vine_synthetic")) {
  ...
  } else if (identical(pretrain_data_mode, "mixed_historical_synthetic")) {
    require_protocol(isTRUE(metadata$mixed_pretraining_bundle),
                     "mixed_pretraining_bundle is not TRUE")
    ...
    require_protocol(identical(as.integer(metadata$synthetic_unique_episode_count), 100L),
                     "synthetic_unique_episode_count is not 100")
    require_protocol(identical(as.integer(metadata$historical_unique_episode_count), 61L),
                     "historical_unique_episode_count is not 61")
    require_protocol(identical(as.integer(metadata$mixed_episode_presentations), 1000L),
                     "mixed_episode_presentations is not 1000")
    require_protocol(isFALSE(metadata$evaluation_data_accessed),
                     "mixed bundle accessed evaluation data")
  } else {
...
if (pretrain_episodes != length(pretrain_returns)) {
  stop("PRETRAIN_EPISODES must equal the registered pretraining-presentation count.")
}
