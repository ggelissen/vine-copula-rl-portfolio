...
clean_checkpoint_attestation <- FALSE
if (reuse_completed_training) {
  attestation_file <- file.path(run_root, "checkpoint_evidence.json")
  if (file.exists(attestation_file)) {
    attestation <- yaml::yaml.load_file(attestation_file)
    clean_checkpoint_attestation <-
      identical(as.character(attestation$status),
                "valid_embedded_no_vine_checkpoint_evidence") &&
      identical(as.integer(attestation$seed_count), all_seed_count) &&
      identical(as.integer(attestation$checkpoint_count), 2L * all_seed_count) &&
      identical(as.character(attestation$expected_mode), "zero") &&
      identical(as.character(attestation$expected_signal_mask),
...
for (index in seq_along(seeds)) {
  ...
  if (reuse_completed_training) {
    required_training_files <- file.path(output_dir, c(
      "td3_lstm_vine_pretrained.pt", "td3_lstm_vine_full.pt",
      "run_manifest.rds", "vine_observation_mode.txt"))
    if (any(!file.exists(required_training_files))) {
      stop("Reuse mode lacks required completed-training evidence for seed ",
           seed, ": ",
           paste(basename(required_training_files[!file.exists(
             required_training_files)]), collapse = ", "))
    }
    repair_record <- file.path(output_dir, "vine_observation_mode_repair.json")
    if (!clean_checkpoint_attestation && !file.exists(repair_record)) {
      stop("Reuse mode has neither an embedded checkpoint attestation nor a ",
           "legacy repair record for seed ", seed)
    }
    manifest <- readRDS(file.path(output_dir, "run_manifest.rds"))
    if (clean_checkpoint_attestation &&
        (!identical(as.character(manifest$interventions$vine_observation_mode),
                    "zero") ||
         !identical(as.character(manifest$interventions$no_vine_signal_mask),
                    "explicit_vine_and_scenario_cvar_v1"))) {
      stop("Clean-training manifest intervention mismatch for seed ", seed)
    }
