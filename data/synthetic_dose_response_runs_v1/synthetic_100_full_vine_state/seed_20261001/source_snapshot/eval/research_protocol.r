...
validate_weight_log <- function(x, periods, asset_names, net_exposure,
                                gross_leverage, max_long_weight = 0.60,
                                max_short_weight = 0.20, tolerance = 1e-6) {
  x <- as.data.table(x)
  ...
  if (any(weights > max_long_weight + tolerance) ||
      any(weights < -max_short_weight - tolerance)) {
    stop("A strategy violates the common single-position constraints.")
  }
  weights
}
...
score_weight_log <- function(weights, asset_gross, periods, asset_names,
  ...
  for (t in seq_len(nrow(weights))) {
    ...
    holding_year_fraction <- as.numeric(
      as.Date(periods$holding_end_date[t]) -
        as.Date(periods$decision_date[t])) / 365
    ...
    previous <- w * asset_gross[t, ] / gross_portfolio_return
  }
  rbindlist(rows)
}
...
run_research_evaluation <- function(returns_xts, strategy_weight_logs,
  ...
  scored <- lapply(names(strategy_weight_logs), function(name) {
    ...
    weights <- validate_weight_log(log, periods, asset_names, net_exposure,
                                   gross_leverage, max_long_weight,
                                   max_short_weight)
