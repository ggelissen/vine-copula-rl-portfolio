  ...
  public = list(
    ...
    initialize = function(marginals, asset_names, 
                          ...
                          sim_cores = 1L,
                           ...
                           vine_feature_mode = NULL,
                           cvar_observation_mode = NULL,
                           cvar_reward_mode = c("full", "zero"),
      ...
      # Keep the legacy joint switch for exact replay of schema-5 checkpoints,
      # while exposing orthogonal switches for causal component ablations.
      # Every mask preserves observation width and therefore policy capacity.
      private$vine_observation_mode <- match.arg(vine_observation_mode)
      private$vine_feature_mode <- if (is.null(vine_feature_mode)) {
        private$vine_observation_mode
      } else {
        match.arg(vine_feature_mode, c("full", "zero"))
      }
      private$cvar_observation_mode <- if (is.null(cvar_observation_mode)) {
        private$vine_observation_mode
      } else {
        match.arg(cvar_observation_mode, c("full", "zero"))
      }
      private$cvar_reward_mode <- match.arg(cvar_reward_mode)
      if (identical(private$vine_observation_mode, "zero") &&
          (!identical(private$vine_feature_mode, "zero") ||
           !identical(private$cvar_observation_mode, "zero"))) {
        stop("Legacy VINE_OBSERVATION_MODE=zero requires both policy-visible signals to be zero.")
      }
    ...
    reset = function() {
      ...
      private$holding_year_fractions <- rep(1 / 12, private$T)
      ...
      if (!is.null(private$precomputed_returns)) {
        ...
        if (!is.null(episode$holding_year_fractions)) {
          fractions <- as.numeric(episode$holding_year_fractions)
          if (length(fractions) != private$T || any(!is.finite(fractions)) ||
              any(fractions <= 0) || any(fractions > 1)) {
            stop("Episode holding_year_fractions must contain T valid annual fractions.")
          }
          private$holding_year_fractions <- fractions
        }
    ...
    step = function(action) {
      ...
      if (!is.null(private$precomputed_returns)) {
        ...
        if (is.list(episode) && !is.null(episode$returns)) {
          ...
          holding_year_fraction <-
            private$holding_year_fractions[private$precomputed_step]
      ...
      if (!exists("holding_year_fraction", inherits = FALSE)) {
        holding_year_fraction <- private$holding_days / 252
      }
      financing_cost <- (private$short_borrow_rate * short_notional +
                         private$cash_borrow_rate * cash_borrow_notional) *
        holding_year_fraction
      ...
      # ---- 4. Compute next period's drifted pre-trade holdings ----
      # The realised asset move changes the risky book before the next
      # rebalance. Turnover is therefore target-versus-drifted-holdings, not
      # target-versus-prior-target. Implicit cash is included in portf_ret.
      private$previous_action <- action_vec * R / portf_ret
      if (any(!is.finite(private$previous_action))) {
        stop("Realised returns produced non-finite drifted holdings.")
      }
      ...
      reward_cvar <- if (identical(private$cvar_reward_mode, "zero")) 0 else cvar
      reward <- utility_increment - private$lambda * reward_cvar
      ...
      return(list(
        ...
        info = list(
          ...
          net_portf_ret = net_portf_ret,
           cvar = cvar,
           reward_cvar = reward_cvar,
           cvar_reward_mode = private$cvar_reward_mode,
          turnover = turnover,
    ...
    get_obs = function() {
      # Backward-compatible intervention invariant: the preregistered legacy
      # no-vine treatment masks every policy-visible dependence channel.  The
      # newer component modes remain available when the legacy mode is full.
      ...
      } else if (identical(private$vine_feature_mode, "zero")) {
        numeric(private$vine_dim)
      } else {
        private$vine_state
      }
      # last_cvar is computed from joint scenarios. It is masked independently
      # so the direct vine state and the derived risk statistic have separately
      # identifiable effects; the legacy no-vine control masks both.
      cvar_observation <- if (no_vine_observation) 0 else if (
        identical(private$cvar_observation_mode, "zero")
      ) 0 else private$last_cvar * 100
      obs <- c(
        private$wealth / private$w0,
    ...
    get_signal_modes = function() list(
      vine_observation_mode = private$vine_observation_mode,
      vine_feature_mode = private$vine_feature_mode,
      cvar_observation_mode = private$cvar_observation_mode,
      cvar_reward_mode = private$cvar_reward_mode)
  ),
