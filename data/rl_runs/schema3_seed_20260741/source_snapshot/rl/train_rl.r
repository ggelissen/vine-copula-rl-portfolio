...
pretrain_max_mean_abs_weight <- as.numeric(Sys.getenv("PRETRAIN_MAX_MEAN_ABS_WEIGHT", "0.75"))
...
if (any(is.na(c(train_seed, output_dir, device, n_sim_cvar, vine_sim_cores, L, ref_col,
                ...
                pretrain_max_mean_abs_weight, pretrain_max_mean_turnover,
                ...
                entropy_coef, direction_logit_bound, initial_leverage_gate,
...
if (any(c(pretrain_max_mean_leverage_gate,
          pretrain_max_mean_gross_cap_fraction) <= 0) ||
    any(c(pretrain_max_mean_leverage_gate,
          pretrain_max_mean_gross_cap_fraction) > 1) ||
    pretrain_max_mean_abs_weight <= 0 || pretrain_max_mean_turnover <= 0) {
...
PRETRAIN_MAX_MEAN_ABS_WEIGHT = float(os.environ.get('PRETRAIN_MAX_MEAN_ABS_WEIGHT', '0.75'))
...
class LSTMActor(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden=128, num_layers=2):
        ...
        self.fc = nn.Sequential(
            ...
            nn.ReLU(),
            # Independent long and short score books plus one explicit
            # leverage gate.  Capped-simplex projection below prevents either
            # book from collapsing into a single asset.
            nn.Linear(hidden, 2 * int(action_dim) + 1)
        )
...
def action_components(raw_action):
    # Independent long/short books can express diversified full leverage.  A
    # capped-simplex projection then enforces institutional position limits
    # without returning to a long-only simplex.
    action_dim = (raw_action.shape[-1] - 1) // 2
    raw_long_logits = raw_action[..., :action_dim]
    raw_short_logits = raw_action[..., action_dim:2 * action_dim]
    long_logits = DIRECTION_LOGIT_BOUND * torch.tanh(
        raw_long_logits / DIRECTION_LOGIT_BOUND)
    short_logits = DIRECTION_LOGIT_BOUND * torch.tanh(
        raw_short_logits / DIRECTION_LOGIT_BOUND)
    leverage_gate = torch.sigmoid(raw_action[..., -1:])
    return long_logits, short_logits, leverage_gate

def capped_simplex(logits, cap):
    # Euclidean projection onto {p >= 0, sum(p)=1, p_i <= cap}.  Bisection is
    # deterministic and piecewise differentiable for the seven-asset problem.
    ...
    probabilities = torch.clamp(logits - 0.5 * (lower + upper), min=0.0, max=cap)
    return probabilities / probabilities.sum(dim=-1, keepdim=True).clamp_min(1e-12)

def portfolio_weights(raw_action):
    # Differentiable self-financing allocation with an explicit leverage gate.
    # gate=0 is neutral net exposure; gate=1 approaches the admissible
    # long-short boundary subject to the bounded concentration geometry.
    long_logits, short_logits, leverage_gate = action_components(raw_action)
    ...
    neutral = torch.full_like(full_risk, NET_EXPOSURE / long_logits.shape[-1])
    return neutral + leverage_gate * (full_risk - neutral)

def allocation_entropy(raw_action):
    long_logits, short_logits, _ = action_components(raw_action)
    long_budget = 0.5 * (GROSS_LEVERAGE + NET_EXPOSURE)
    short_budget = 0.5 * (GROSS_LEVERAGE - NET_EXPOSURE)
    long_probs = capped_simplex(long_logits, MAX_LONG_WEIGHT / long_budget)
    short_probs = (capped_simplex(short_logits, MAX_SHORT_WEIGHT / short_budget)
                   if short_budget > 0 else torch.zeros_like(long_probs))
    ...
    def select_action(self, state_seq, noise_scale=0.0):
        ...
        with torch.no_grad():
            ...
            if self.total_actions < self.random_exploration_steps:
                raw_action = torch.randn((state_tensor.shape[0], 2 * self.action_dim + 1), device=device)
            else:
        ...
        long_logits, short_logits, gate = action_components(raw_action)
        projected = portfolio_weights(raw_action)
        long_budget = 0.5 * (GROSS_LEVERAGE + NET_EXPOSURE)
        short_budget = 0.5 * (GROSS_LEVERAGE - NET_EXPOSURE)
        long_probs = capped_simplex(long_logits, MAX_LONG_WEIGHT / long_budget)
        short_probs = (capped_simplex(short_logits, MAX_SHORT_WEIGHT / short_budget)
        ...
        entropy = -0.5 * (
            ...
            torch.sum(short_probs * torch.log(short_probs + 1e-8), dim=-1))
        self.last_action_diagnostics = {
    ...
    def update(self, replay_buffer, batch_size=32):
        ...
        if self.update_count % POLICY_DELAY == 0:
            with torch.amp.autocast('cuda', enabled=(USE_AMP and device.type == 'cuda')):
                ...
                pred_action = portfolio_weights(pred_raw_action_last)
                q_value, _ = self.critic(states, pred_action)
                actor_loss = -q_value.mean()
                if self.entropy_coef != 0.0:
                    actor_loss = actor_loss - self.entropy_coef * allocation_entropy(pred_raw_action_last)
            ...
            with torch.no_grad():
                _, _, diagnostic_gate = action_components(pred_raw_action_last)
                actor_gate_tensor = diagnostic_gate.mean()
                actor_entropy_tensor = allocation_entropy(pred_raw_action_last).mean()
...
def train_stage(env, agent, episodes, stage, batch_size=32, noise_scale=0.0,
    ...
    for ep in range(episodes):
        ...
        leverage_gate_values, direction_entropy_values = [], []
...
def enforce_pretraining_behavior_gate():
    pretrain_rows = [row for row in training_episode_rows if row['stage'] == 'pretrain']
    if len(pretrain_rows) < PRETRAIN_BEHAVIOR_GATE_WINDOW:
        raise RuntimeError('Too few completed episodes for the pre-training behaviour gate.')
    recent = pretrain_rows[-PRETRAIN_BEHAVIOR_GATE_WINDOW:]
    specifications = [
        ('mean_leverage_gate',
         float(np.mean([row['mean_leverage_gate'] for row in recent])),
         PRETRAIN_MAX_MEAN_LEVERAGE_GATE),
        ('fraction_at_gross_cap',
         float(np.mean([row['fraction_at_gross_cap'] for row in recent])),
         PRETRAIN_MAX_MEAN_GROSS_CAP_FRACTION),
        ('max_abs_weight',
         float(np.mean([row['max_abs_weight'] for row in recent])),
         PRETRAIN_MAX_MEAN_ABS_WEIGHT),
        ('mean_turnover',
         float(np.mean([row['mean_turnover'] for row in recent])),
         PRETRAIN_MAX_MEAN_TURNOVER)]
    gate_rows = [{
        'window_episodes': PRETRAIN_BEHAVIOR_GATE_WINDOW,
        'metric': metric, 'value': value, 'maximum': maximum,
        'pass': bool(np.isfinite(value) and value <= maximum)}
        for metric, value, maximum in specifications]
    ...
    if failures:
        details = '; '.join(
            f\"{row['metric']}={row['value']:.6g} > {row['maximum']:.6g}\"
            for row in failures)
        raise RuntimeError(
            'Pre-training behavioural gate failed; historical fine-tuning is locked: ' +
            details)
    log_print('PRE-TRAINING BEHAVIOUR GATE PASSED: ' + ', '.join(
        f\"{row['metric']}={row['value']:.4f}\" for row in gate_rows))
...
log_print(f'Action schema: capped_two_book_leverage_gate_v3 | gross={GROSS_LEVERAGE:.3f} | net={NET_EXPOSURE:.3f} | max_long={MAX_LONG_WEIGHT:.3f} | max_short={MAX_SHORT_WEIGHT:.3f}')
log_print(f'Risk/training: entropy_coef={ENTROPY_COEF:.6g} | direction_bound={DIRECTION_LOGIT_BOUND:.3f} | AMP={USE_AMP}')
log_file.flush()
")
...
enforce_pretraining_behavior_gate()
