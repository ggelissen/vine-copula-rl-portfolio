...
class LSTMActor(nn.Module):
    def __init__(self, obs_dim, action_dim, hidden, num_layers):
        ...
        self.fc = nn.Sequential(nn.Linear(hidden, hidden), nn.ReLU(), nn.Linear(hidden, 2 * action_dim + 1))
...
def load_actor(path, obs_dim, action_dim):
    ...
    expected = {'obs_dim': obs_dim, 'action_dim': action_dim,
                'actor_output_dim': 2 * action_dim + 1, 'hidden': HIDDEN, 'num_layers': NUM_LAYERS,
