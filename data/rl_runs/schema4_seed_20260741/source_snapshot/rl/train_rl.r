...
pretrain_max_position_cap_fraction <- as.numeric(Sys.getenv("PRETRAIN_MAX_POSITION_CAP_FRACTION", "0.75"))
...
if (any(is.na(c(train_seed, output_dir, device, n_sim_cvar, vine_sim_cores, L, ref_col,
                ...
                pretrain_max_position_cap_fraction, pretrain_max_gate_gross_mae,
...
if (!is.finite(env_net_exposure) || env_net_exposure <= 0 ||
  ...
  stop("Schema-4 rank-partition actions require positive net exposure and gross >= net.")
}
...
if (any(c(pretrain_max_mean_leverage_gate,
          pretrain_max_mean_gross_cap_fraction,
          pretrain_max_position_cap_fraction) <= 0) ||
    any(c(pretrain_max_mean_leverage_gate,
          pretrain_max_mean_gross_cap_fraction,
          pretrain_max_position_cap_fraction) > 1) ||
...
PRETRAIN_MAX_POSITION_CAP_FRACTION = float(os.environ.get('PRETRAIN_MAX_POSITION_CAP_FRACTION', '0.75'))
...
if NET_EXPOSURE <= 0:
    raise RuntimeError('Schema-4 rank-partition actions require positive net exposure.')
...
def action_components(raw_action):
    ...
    raw_direction_logits = raw_action[..., :action_dim]
    direction_logits = DIRECTION_LOGIT_BOUND * torch.tanh(
        raw_direction_logits / DIRECTION_LOGIT_BOUND)
    leverage_gate = torch.sigmoid(raw_action[..., -1:])
    return direction_logits, leverage_gate

def capped_simplex(logits, cap):
    # KL projection of softmax scores onto {p >= 0, sum(p)=1, p_i <= cap}.
    # This avoids the gratuitous zeros/cap-binding produced by the previous
    # Euclidean sparsemax projection while preserving exact position limits.
...
def portfolio_books(raw_action):
    direction_logits, leverage_gate = action_components(raw_action)
    action_dim = direction_logits.shape[-1]
    if action_dim <= SHORT_SUPPORT_SIZE:
        raise RuntimeError('Actor action dimension is incompatible with the short support.')
    ...
    masked_long_logits = torch.where(
        long_mask, direction_logits, torch.full_like(direction_logits, -1e9))
    masked_short_logits = torch.where(
    ...
    long_cap = torch.clamp(MAX_LONG_WEIGHT / long_budget.clamp_min(1e-12), max=1.0)
    long_probs = capped_simplex(masked_long_logits, long_cap)
    if SHORT_SUPPORT_SIZE > 0:
        short_cap = torch.clamp(
            MAX_SHORT_WEIGHT / short_budget.clamp_min(1e-12), max=1.0)
        short_probs = capped_simplex(masked_short_logits, short_cap)
    else:
...
def enforce_pretraining_behavior_gate():
    ...
    specifications = [
        ...
        ('fraction_at_gross_cap',
         ...
         PRETRAIN_MAX_MEAN_GROSS_CAP_FRACTION),
        ('fraction_at_position_cap',
         float(np.mean([row['fraction_at_position_cap'] for row in recent])),
         PRETRAIN_MAX_POSITION_CAP_FRACTION),
        ('gate_gross_mae',
         float(np.mean([row['gate_gross_mae'] for row in recent])),
...
log_print(f'Action schema: rank_partition_leverage_gate_v4 | gross={GROSS_LEVERAGE:.3f} | net={NET_EXPOSURE:.3f} | max_long={MAX_LONG_WEIGHT:.3f} | max_short={MAX_SHORT_WEIGHT:.3f}')
