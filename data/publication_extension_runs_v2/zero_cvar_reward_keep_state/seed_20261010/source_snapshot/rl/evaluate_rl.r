...
if (any(!c(vine_feature_mode, cvar_observation_mode, cvar_reward_mode) %in%
        c("full", "zero"))) stop("Invalid independent vine/CVaR evaluation mode.")
...
if (!identical(as.character(asset_names), colnames(returns))) {
  stop("Checkpoint/vine asset names do not match the active return-data contract.")
}
...
validate_return_evaluation_contract(returns, period_split, T_eval)
...
fit_truncation <- suppressWarnings(as.numeric(nn_fit$truncation_level))
active_fit_trees <- if (length(fit_truncation) == 1L &&
                        is.infinite(fit_truncation) && fit_truncation > 0) {
  length(asset_names) - 1L
} else suppressWarnings(as.integer(fit_truncation))
if (length(active_fit_trees) != 1L || !is.finite(active_fit_trees) ||
    active_fit_trees < 1L || active_fit_trees >= length(asset_names) ||
    as.integer(nn_fit$training_observations) != nrow(copula_innovation_u) ||
    as.integer(nn_fit$dynamic_edge_count) !=
      sum(length(asset_names) - seq_len(active_fit_trees))) {
  stop("Persisted NN vine does not match the locked training split/active-tree architecture. Regenerate it.")
}
...
evaluation_episode <- list(
  ...
  holding_year_fractions = as.numeric(
    eval_periods$holding_end_date - eval_periods$decision_date) / 365,
...
if (supports_vine_observation_mode) {
  ...
  if ("vine_feature_mode" %in% names(formals(environment_initialiser))) {
    environment_arguments$vine_feature_mode <- vine_feature_mode
    environment_arguments$cvar_observation_mode <- cvar_observation_mode
    environment_arguments$cvar_reward_mode <- cvar_reward_mode
  } else if (!identical(vine_feature_mode, vine_observation_mode) ||
             !identical(cvar_observation_mode, vine_observation_mode) ||
             !identical(cvar_reward_mode, "full")) {
    stop("This RLEnvironment cannot reproduce the checkpoint's independent signal modes.")
  }
...
# Select the inference implementation from the checkpoint protocol rather than
# importing Python PyTorch into the R process.  Historical schema-5 LSTM-TD3
# checkpoints retain the frozen v1 path; independent signal interventions,
# non-TD3 algorithms, and non-LSTM encoders require the generalized v2 server.
legacy_policy_server <- "rl/policy_inference_server.py"
extension_policy_server <- "rl/policy_inference_server_v2.py"
policy_algorithm <- tolower(Sys.getenv("RL_ALGORITHM", "td3"))
policy_encoder <- tolower(Sys.getenv("POLICY_ENCODER", "lstm"))
independent_causal_modes <-
  !identical(vine_feature_mode, vine_observation_mode) ||
  !identical(cvar_observation_mode, vine_observation_mode) ||
  !identical(cvar_reward_mode, "full")
requires_extension_server <- independent_causal_modes ||
  !identical(policy_algorithm, "td3") || !identical(policy_encoder, "lstm")
default_policy_server <- if (requires_extension_server) {
  extension_policy_server
} else {
  legacy_policy_server
}
policy_server <- normalizePath(
  Sys.getenv("POLICY_INFERENCE_SERVER", default_policy_server), mustWork = TRUE)
...
checkpoint_prefix <- Sys.getenv("EVAL_CHECKPOINT_PREFIX", "td3_lstm_vine")
if (!grepl("^[A-Za-z0-9_]+$", checkpoint_prefix)) {
  stop("EVAL_CHECKPOINT_PREFIX contains unsafe characters.")
}
all_logs <- as.data.frame(rbindlist(lapply(models_to_run, function(model) {
  run_isolated_policy(file.path(eval_model_dir,
                                paste0(checkpoint_prefix, "_", model, ".pt")), model)
}), fill = TRUE))
