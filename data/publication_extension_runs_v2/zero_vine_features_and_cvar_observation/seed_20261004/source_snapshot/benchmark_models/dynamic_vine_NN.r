...
# For small panels the maximum-dependence Hamiltonian path is searched exactly.
# Factorial enumeration is impossible for the preregistered 18/30-50 asset
# panels, so larger systems use deterministic all-start nearest-neighbour paths
# followed by 2-opt improvements.  Both methods use training ranks only.
select_dvine_order <- function(U) {
  ...
  path_score <- function(path)
    sum(tau[cbind(head(path, -1L), tail(path, -1L))])
  if (d > 9L) {
    improve_path <- function(path) {
      best_score <- path_score(path)
      repeat {
        improved <- FALSE
        # Reversing a contiguous segment is the path analogue of 2-opt.
        for (left in seq_len(d - 2L)) {
          for (right in seq.int(left + 1L, d - 1L)) {
            candidate <- c(path[seq_len(left)],
                           rev(path[seq.int(left + 1L, right)]),
                           path[seq.int(right + 1L, d)])
            score <- path_score(candidate)
            if (score > best_score + 1e-12) {
              path <- candidate; best_score <- score; improved <- TRUE
            }
          }
        }
        if (!improved) break
    ...
    candidates <- lapply(seq_len(d), function(start) {
      path <- start; remaining <- setdiff(seq_len(d), start)
      while (length(remaining)) {
        scores <- tau[tail(path, 1L), remaining]
        # which.max is deterministic and resolves ties by the frozen column order.
        next_node <- remaining[which.max(scores)]
        path <- c(path, next_node); remaining <- setdiff(remaining, next_node)
      }
      improve_path(path)
    })
    scores <- vapply(candidates, path_score, numeric(1))
    return(as.integer(candidates[[which.max(scores)]]))
  }
...
validate_truncation <- function(U, order, candidate_level = NULL,
                                train_fraction = 0.8) {
  ...
  if (is.null(candidate_level)) candidate_level <- ncol(U) - 1L
  candidate_level <- max(1L, min(as.integer(candidate_level), ncol(U) - 1L))
  candidate <- vinecop(train, structure = structure, family_set = "t",
                       trunc_lvl = candidate_level, selcrit = "bic")
  delta <- log(pmax(dvinecop(validation, candidate), 1e-300)) -
           log(pmax(dvinecop(validation, truncated), 1e-300))
  se <- sd(delta) / sqrt(length(delta))
  list(mean_candidate_minus_tree1 = mean(delta), candidate_level = candidate_level,
       mean_full_minus_truncated = if (candidate_level == ncol(U) - 1L)
         mean(delta) else NA_real_, standard_error = se,
...
compute_dvine_edge_data <- function(U, vine) {
  ...
  if (n_trees < 1L || n_trees > d - 1L) stop("Invalid D-vine truncation level.")
  edge_data <- vector("list", n_trees)
...
train_all_edges <- function(U, vine_fit, asset_names, z, sigma,
  ...
  n_trees <- length(vine_fit$pair_copulas)
  nn_models <- vector("list", n_trees)
  for (tree in seq_len(n_trees)) {
...
build_nn_vine <- function(nn_models, full_vine, U_window,
  ...
  n_trees <- length(full_vine$pair_copulas)
  if (length(nn_models) != n_trees) stop("NN model does not cover every active vine tree.")
...
fit_nn_dynamic_vine <- function(U, z = NULL, sigma = NULL, epochs = 200L,
  ...
  active_trees <- if (is.finite(truncation_level) && truncation_level > 0) {
    min(as.integer(truncation_level), ncol(U) - 1L)
  } else {
    ncol(U) - 1L
  }
  truncation_validation <- validate_truncation(
    U, order, candidate_level = active_trees)
  backbone <- vinecop(U, var_types = rep("c", ncol(U)),
    structure = dvine_structure(order), family_set = "t",
    trunc_lvl = active_trees, selcrit = "bic")
  ...
  list(backbone = backbone, nn_models = nn_models, order = order,
       truncation_level = active_trees,
       dynamic_edge_count = sum(ncol(U) - seq_len(active_trees)),
       truncation_validation = truncation_validation,
       order_search = if (ncol(U) <= 9L) "exact_hamiltonian_path" else
         "deterministic_all_start_greedy_2opt",
       model = if (active_trees == ncol(U) - 1L)
         "nn_dynamic_all_tree_t_vine" else "nn_dynamic_truncated_t_vine",
       training_observations = nrow(U), seed = seed)
}
...
load_nn_dynamic_vine_fit <- function(dir_path) {
  ...
  if (!fit$model %in% c("nn_dynamic_all_tree_t_vine",
                        "nn_dynamic_truncated_t_vine")) {
