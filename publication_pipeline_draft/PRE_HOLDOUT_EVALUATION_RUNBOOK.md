...
Evaluation release v2 is an operational revision after two archived attempts
failed before producing policy weights: R Lantern and Python PyTorch exported
incompatible libtorch symbols in one process. The v2 evaluator runs the
...
python3 publication_pipeline_draft/freeze_evaluation_release.py \
  ...
  --output frozen_releases/evaluation_main_v2 \
  --bundle frozen_releases/evaluation_main_v2.tar.gz

(cd frozen_releases && sha256sum -c evaluation_main_v2.tar.gz.sha256)
(cd frozen_releases/evaluation_main_v2 && sha256sum -c CONTENTS.sha256)
python3 -m json.tool frozen_releases/evaluation_main_v2/evaluation_release_manifest.json
```
...
/gabirel/miniforge3/bin/python3 \
  publication_pipeline_draft/locked_evaluation_batch.py \
  --repo-root . \
  --evaluation-release frozen_releases/evaluation_main_v2 \
  --output locked_evaluation/main_oos_v2_operational_retry \
  --bundle locked_evaluation/main_oos_v2_operational_retry.tar.gz \
  --rscript /gabirel/miniforge3/bin/Rscript

(cd locked_evaluation && sha256sum -c main_oos_v2_operational_retry.tar.gz.sha256)
python3 -m json.tool locked_evaluation/main_oos_v2_operational_retry/locked_batch_manifest.json
```

Only after the bundle and sidecar hash exist should tables and plots under
`locked_evaluation/main_oos_v2_operational_retry/publication_results` be inspected.
