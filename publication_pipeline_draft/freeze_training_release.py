...
def validate_diagnostic_archive(
    ...
    if not archive.is_file():
        raise ProtocolError(f"Training diagnostic archive not found: {archive}")
...
def deterministic_tar(source: Path, bundle: Path) -> None:
    if bundle.exists():
        raise ProtocolError(f"Bundle already exists and will not be overwritten: {bundle}")
    bundle.parent.mkdir(parents=True, exist_ok=True)
    with tarfile.open(bundle, "w:gz", format=tarfile.PAX_FORMAT) as handle:
...
def freeze_training_release(
    ...
    try:
        ...
        snapshot_inventory = verify_training_snapshot(
            tables["hash_consensus"], repo_root, temporary, verify_data, copy_data
        )
        ...
        release_manifest = {
            ...
            "next_gate": "freeze evaluation contract and benchmark implementations before one locked OOS batch",
        }
        ...
        (temporary / "READ_ONLY_RELEASE.txt").write_text(
            "This directory is the frozen pre-OOS training release. Do not edit files in place.\n"
