...
NARRATIVE = r"""% Additive authoring fragment. It does not modify the current Results chapter.
...
def generate(context: PublicationContext) -> None:
    inputs = [
        context.input("primary_economic_metrics.csv"),
        context.input("daily_tail_risk_metrics.csv"),
        context.input("registered_contrast_robustness_summary.csv"),
        context.input("white_reality_checks.csv"),
        context.input("friction_surface.csv"),
    ]
    context.write_text(
        "narrative/terminal_results_narrative.tex", NARRATIVE,
        artifact_type="latex_narrative_fragment",
        title="Terminal robustness results narrative",
