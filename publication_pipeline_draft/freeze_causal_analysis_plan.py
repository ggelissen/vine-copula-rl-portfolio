...
def freeze(repo_root: Path, extension_release: Path, contract_path: Path,
           ...
           operational_merge_manifest: Path | None = None) -> dict[str, Any]:
    ...
    extension = verify_extension_release(extension_release.resolve(), repo_root)
    ...
    if carried_extension_release is not None or operational_merge_manifest is not None:
        ...
        if (releases.get("original", {}).get("contents_sha256") !=
