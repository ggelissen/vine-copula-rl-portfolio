...
def test_replay_is_audited_weights_only_and_analysis_reuses_candidate_hash() -> None:
    ...
    assert 'row["annual_volatility_candidate"]' in analysis
    assert "annualized_volatility_candidate" not in analysis
    assert '"operational_revision": "v1_1_metric_column_name_correction"' in analysis
    assert '"scientific_contract_changed": False' in analysis
